...
class particle implements nonWorldElems {
    x: number // x coordinate
    y: number // y coordinate
    entity: entity
    spriteWidth: number
...
abstract class Entity {
    ...
    seeRange: number
    worldElem: worldElementNames
    constructor(x: number, y: number, maxHealth: number, attackRange: number, spriteWidth: number, spriteHeight: number, attackDamage: number, animationStates: AnimationState[], img: HTMLImageElement, type: typeObject, scale: number, lootDrop: { amount: number, drop: item, chance: number }[], worldElem: worldElementNames, name: string) {
        this.x = x
        this.y = y
        this.frames = 0
        this.frameLoc = 0
...
class nightBorn extends Entity implements entity {
    worldElem: worldElementNames
    constructor(x: number, y: number, worldElem: worldElementNames) {
        let image = new Image()
...
class trader extends Entity implements entity {
    trade: Trade[][]
    worldElem: worldElementNames
    constructor(x: number, y: number, trade: Trade[][], worldElem: worldElementNames) {
        let image = new Image()
...
class NPC extends Entity implements entity {
    ...
    constructor(x: number, y: number, worldElem: worldElementNames, pathToImage: string, spriteWidth: number, spriteHeight: number, frameAmount: number, scale: number, conversation: String[], name: string, present: PresentItem[]) {
        let image = new Image()
    ...
    attack(): void {
        ...
        if (!this.onCooldown) {
            if (this.onGround) {
                ...
                setTimeout(() => {
                    entities.forEach(obj => { // get each entity in range of the attack
...
// world logic
type worldElementNames = /* 'bush_fruit' | */ 'wall_2' | 'wall_1' | 'goblin' | 'nightBorn' | 'skeleton' | 'tree_1' | 'tree_2' | 'rocks_1' | 'bush_1' | 'bush_2' | 'bush_3' | 'plant_1' | 'statue_1' | 'chest' | 'NPC'

const worldElements: Record<worldElementNames, any> = {
