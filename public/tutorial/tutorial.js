...
let levelObjs = [
    ...
    { pos: 675, type: 'goblin' },
    { pos: 700, type: 'goblin' },
    { pos: 1000, type: 'goblin' },
    { pos: 1300, type: 'goblin' },
    { pos: 1400, type: 'goblin' },
    { pos: 1800, type: 'goblin' },
    { pos: 2000, type: 'goblin' }
];
let levelPos = 0;
// sound logic
...
addEventListener('keydown', event => {
    if (!player.onInventory && !player.onTradingMenu && !player.onSecondaryInventory && player.canMove)
...
class Layer {
    constructor(image, speedModifier, spriteWidth, spriteHeight) {
        this.x = 0;
        this.y = 0;
...
class particle {
    ...
    draw() {
        ...
        ctx.drawImage(this.img, frameX, frameY, this.spriteWidth, this.spriteHeight, this.x, this.y, 150 * ((_a = this.entity.scale) !== null && _a !== void 0 ? _a : 1), 150 * ((_b = this.entity.scale) !== null && _b !== void 0 ? _b : 1));
    }
    interact() {
        return;
    }
...
class healthbar {
    ...
    draw() {
        if (!this.entity.maxHealth || !this.entity.health)
            return;
        const backgroundColor = this.entity.type.name === 'interactable' ? "rgb(215, 215, 215)" : "rgb(184, 0, 0)";
        const overColor = this.entity.type.name === 'interactable' ? "rgb(101, 101, 101)" : "rgb(0, 184, 3)";
        let scale = 1;
...
class interactable {
    constructor(x, y, cooldown, pathToImage, spriteWidth, spriteHeight, scale, output, isInfinite, healthBarScale, worldElem, canBeInteracted, isBlocking, removeItem) {
        this.x = x;
        this.y = y;
        this.spriteWidth = spriteWidth;
        this.spriteHeight = spriteHeight;
    ...
    interact() {
        if (this.wasCollected && !this.isInfinite)
            return;
...
// enemy classes
class Entity {
    constructor(x, y, maxHealth, attackRange, spriteWidth, spriteHeight, attackDamage, animationStates, img, type, scale, lootDrop, worldElem, name) {
        this.x = x;
        this.y = y;
        this.frames = 0;
        this.frameLoc = 0;
    ...
    update() {
        ...
        this.effects.forEach(effect => {
            ...
            if (effect.duration <= 0) {
                if (effect.effect.name === 'deaths_curse')
                    this.health -= 100;
        ...
        if (this.type.interactable) {
            ...
            if (this.type.name === 'trader')
                distanceXToPlayer - 50;
            if (distanceXToPlayer <= player.interactionRange && !player.onCooldown && player.onGround) {
                if (!this.showedText) {
                    displayInfo('Press "R" to interact');
                    this.showedText = true;
                }
                if (this.type.name === 'trader' && this.currentState !== 'greet' && this.currentState !== 'open' && this.currentState !== 'dialogue')
                    this.changeState('greet');
...
class nightBorn extends Entity {
    constructor(x, y, worldElem) {
        ...
        }, scale, [{ amount: 1, drop: "silver_ingot", chance: 100 }], worldElem, 'nightborn'); // grabs x and y from the parent class
        this.worldElem = worldElem;
        this.init();
    }
    interact() {
...
class goblin extends Entity {
    constructor(x, y, worldElem) {
        ...
        }, scale, [{ amount: 2, drop: "leather", chance: 50 }, { amount: 1, drop: "cloth", chance: 10 }, { amount: 1, drop: "string", chance: 25 }], worldElem, 'goblin');
        this.worldElem = worldElem;
        this.init();
    }
    interact() {
...
class skeleton extends Entity {
    constructor(x, y, worldElem) {
        ...
        }, scale, [{ amount: 1, drop: "stone", chance: 75 }, { amount: 1, drop: "stone", chance: 75 }], worldElem, 'skeleton'); // grabs x and y from the parent class
        this.worldElem = worldElem;
        this.init();
    }
    interact() {
...
class trader extends Entity {
    constructor(x, y, trade, worldElem) {
        let image = new Image();
...
class NPC extends Entity {
    constructor(x, y, worldElem, pathToImage, spriteWidth, spriteHeight, frameAmount, scale, conversation, name, present) {
        let image = new Image();
...
class Player {
    ...
    update() {
        ...
        if (!this.onGround && !(gameFrame % staggerFrames)) {
            if (this.Ydirec === 1) { // if he is moving upward 
                ...
                if (this.currentState !== 'attack3' && this.currentState !== 'jump')
                    player.changeState('jump');
            }
            else if (this.Ydirec === 2) {
                this.y += this.velocity_Y * globalGravity; // calculate new y position
                this.velocity_Y += 0.2; // advance velocity
                if (this.currentState !== 'attack3' && this.currentState !== 'fall')
    ...
    takeHit(damage) {
        if (this.currentState !== 'attack1' && this.currentState !== 'attack3')
            this.changeState('take_hit');
        let protection = 100;
            ...
            if (this.onGround) {
                ...
                setTimeout(() => {
                    entities.forEach(obj => {
                        ...
                        let rangeValue = ((_a = item === null || item === void 0 ? void 0 : item.attackRange) !== null && _a !== void 0 ? _a : 150) * this.attackRangeMultiplier;
                        if (this.Xdirec === 2)
                            rangeValue + 30;
                        ...
                        if (((this.Xdirec === 1 && distance >= 0) || (distance < 0 && this.Xdirec === 2)) && Math.abs(distance) <= rangeValue && obj.type.attackable) { // Xdirec === 1 --> rechts Xdirec === 2 --> Links
                            ...
                            if (this.checkEffect('strength').wasFound) {
                                attackDamage += (((_b = this.checkEffect('strength').effect) === null || _b === void 0 ? void 0 : _b.factor) / 2) * 6;
                            }
                ...
                if (this.Ydirec !== 0) {
                    ...
                    setTimeout(() => {
                        entities.forEach(obj => {
                            ...
                            const itemKey = this.inventory[3][this.selectedSlot - 1];
                            const item = itemKey ? items[itemKey] : null;
                            let rangeValue = ((_a = item === null || item === void 0 ? void 0 : item.attackRange) !== null && _a !== void 0 ? _a : 150) * this.attackRange3Multiplier;
                            if (this.Xdirec === 2)
                                rangeValue + 30;
                            const distance = (obj.x + (obj.spriteWidth / 2)) - (this.x + (this.spriteWidth / 2));
                            if (((this.Xdirec === 1 && distance >= 0) || (distance < 0 && this.Xdirec === 2)) && Math.abs(distance) <= rangeValue && obj.type.attackable) { // Xdirec === 1 --> rechts Xdirec === 2 --> Links
    ...
    useItem() {
        ...
        if (!currentItem)
            return;
        if (items[player.inventory[3][this.selectedSlot - 1]].clearsAfterUse)
            player.inventory[3][this.selectedSlot - 1] = null;
    ...
    interact() {
        if (this.interactionFocusContainer)
            this.interactionFocusContainer.interact();
        else if (this.interactionFocusEntity)
            this.interactionFocusEntity.interact();
        else if (this.interactionFocusGrab)
            this.interactionFocusGrab.interact();
    }
    addItem(item, amount) {
        let itemCounter = 0;
    ...
    removeEffect(index) {
        for (let i = 0; i < this.effects.length; i++) {
            if (this.effects[i].index === index) {
                ...
                if (this.effects[i].effect.name === 'healthboost')
                    this.maxHealth = this.maxHealth / 1.5;
                this.effects.splice(i, 1);
                break;
            }
        }
...
function renderInventory() {
    ...
    for (let i = 0; i < 3; i++) {
        if (player.armor[i] !== null) {
            ...
            itemDiv.addEventListener('click', (e) => {
                var _a;
                e.stopPropagation();
                player.dragging = e.target.parentElement.id;
    ...
    document.querySelectorAll('.primarySlot').forEach(slot => {
        slot.addEventListener('click', e => {
            ...
            if (player.inventory[targetSlot.y][targetSlot.x] !== null)
                return;
            ...
            renderInventory();
            if (player.onSecondaryInventory)
                renderSecondaryContainer(player.interactionFocusContainer);
        });
    });
    document.querySelectorAll('.armorSlot').forEach(slot => {
        slot.addEventListener('click', e => {
            ...
            const dragSlot = parseSlotId(player.dragging);
            const targetSlot = parseSlotId(targetElement.id);
            if (player.armor[targetSlot.x] !== null)
                return;
            if (player.dragging.slice(0, 13) === 'secondarySlot') {
                const temp = player.interactionFocusContainer.inventory[dragSlot.y][dragSlot.x];
    ...
    document.querySelectorAll('.crafting-slot').forEach(slot => {
        slot.addEventListener('click', e => {
            ...
            const dragSlot = parseSlotId(player.dragging);
            const targetSlot = parseSlotId(targetElement.id);
            if (player.craftingInventory[targetSlot.y][targetSlot.x] !== null)
                return;
            if (player.dragging.slice(0, 13) === 'secondarySlot') {
                const temp = player.interactionFocusContainer.inventory[dragSlot.y][dragSlot.x];
                player.interactionFocusContainer.inventory[dragSlot.y][dragSlot.x] = null;
...
function renderSecondaryContainer(container) {
    ...
    document.querySelectorAll('.secondarySlot').forEach(slot => {
        slot.addEventListener('click', e => {
            ...
            if (player.interactionFocusContainer.inventory[targetSlot.y][targetSlot.x] !== null)
                return;
...
function update() {
    ...
    deadObjects.forEach(obj => {
        ...
        entities.splice(entities.indexOf(obj), 1);
    });
    // reset deadobjects
    deadObjects = [];
    backgroundLayers.forEach(Layer => {
        if ((keys['KeyD'] || keys['KeyW']) && Layer.type.moving === true) {
            Layer.x -= gameSpeed;
        }
        else if ((keys['KeyA'] || keys['KeyS']) && Layer.type.moving === true) {
            Layer.x -= gameSpeed;
        }
        Layer.update();
        Layer.draw();
    ...
    blocks.forEach(block => {
        if ((keys['KeyD'] || keys['KeyW']) && block.type.moving === true) {
            block.x -= gameSpeed;
        }
        else if ((keys['KeyA'] || keys['KeyS']) && block.type.moving === true) {
            block.x -= gameSpeed;
        }
        block.update();
        block.draw();
    ...
    containers.forEach(container => {
        if ((keys['KeyD'] || keys['KeyW']) && container.type.moving === true) {
            container.x -= gameSpeed;
        }
        else if ((keys['KeyA'] || keys['KeyS']) && container.type.moving === true) {
            container.x -= gameSpeed;
        }
        container.update();
        container.draw();
    ...
    entities.forEach(entity => {
        if ((keys['KeyD'] || keys['KeyW']) && entity.type.moving === true) {
            entity.x -= gameSpeed;
        }
        else if ((keys['KeyA'] || keys['KeyS']) && entity.type.moving === true) {
            entity.x -= gameSpeed;
        }
        entity.update();
        entity.draw();
    ...
    particles.forEach(particle => {
        if ((keys['KeyD'] || keys['KeyW']) && particle.type.moving === true) {
            particle.x -= gameSpeed;
        }
        else if ((keys['KeyA'] || keys['KeyS']) && particle.type.moving === true) {
            particle.x -= gameSpeed;
        }
        particle.update();
        particle.draw();
    ...
    let summonedElements = [];
    worlds[currentWorld].elements.forEach((element, i) => {
        const VIEW_LEFT = levelPos;
        const VIEW_RIGHT = levelPos + CANVAS_WIDTH;
        if (element.x >= VIEW_LEFT - 200 && element.x <= VIEW_RIGHT + 200) {
            if (element.elementType === 'blocks') {
                if (element.y) {
                    blocks.push(new element.type.class(element.x - levelPos, element.y, ...element.type.args));
                }
                else {
                    blocks.push(new element.type.class(element.x - levelPos, ...element.type.args));
                }
            }
            else if (element.elementType === 'entities') {
                if (element.y) {
                    entities.push(new element.type.class(element.x - levelPos, element.y, ...element.type.args, ...element.extraArgs));
                }
                else {
                    entities.push(new element.type.class(element.x - levelPos, ...element.type.args));
                }
            }
            else if (element.elementType === 'containers') {
                containers.push(new element.type.class(element.x - levelPos, element.type.args[0], element.extraArgs[0], element.type.args[1]));
            }
            summonedElements.push(i);
        }
    });
    summonedElements
        .sort((a, b) => b - a)
        .forEach(index => {
        worlds[currentWorld].elements.splice(index, 1);
    });
    gameFrame++;
...
const worlds = {
    jungle: {
        ...
        elements: [
            { x: -1700, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: -1800, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: -1000, type: worldElements.tree_1, elementType: 'blocks', extraArgs: [] },
            { x: 3200, type: worldElements.rocks_1, elementType: 'blocks', extraArgs: [] },
            { x: -400, type: worldElements.bush_1, elementType: 'blocks', extraArgs: [] },
            { x: 400, type: worldElements.bush_2, elementType: 'blocks', extraArgs: [] },
            { x: 150, type: worldElements.bush_1, elementType: 'blocks', extraArgs: [] },
            //{ x: -600, type: worldElements.bush_fruit, elementType: 'blocks', extraArgs: [] },
            { x: 900, type: worldElements.bush_3, elementType: 'blocks', extraArgs: [] },
            { x: -1300, type: worldElements.wall_2, elementType: 'blocks', extraArgs: [] },
            { x: 2300, type: worldElements.bush_3, elementType: 'blocks', extraArgs: [] },
            { x: 1500, type: worldElements.statue_1, elementType: 'blocks', extraArgs: [] },
            { x: -200, type: worldElements.chest, elementType: 'containers', extraArgs: [[['stick', null, null, 'horn', null], [null, null, 'horn', null, null], [null, null, null, null, null], ['iron_ingot', null, null, null, null]]] },
            { x: 1000, type: worldElements.tree_1, elementType: 'blocks', extraArgs: [] },
            { x: 750, y: 545, type: worldElements.NPC, elementType: 'entities', extraArgs: ['/img/passiveEntities/elder.png', 32, 32, 4, 0.4, ['Ohh, welcome traveler!', 'Isn\'t that statue beautifull!', 'What, you don\'t know anything??', 'Well, that\'s probably for the best.', 'The goblin king has declared war on Norwyn!', 'You need to rescue the captured humans!', 'I mean you do look like a strong warrior!', 'So what are you waiting for and go right to find them!'], 'elder', []] },
            { x: 2500, y: 430, type: worldElements.NPC, elementType: 'entities', extraArgs: ['/img/passiveEntities/samurai.png', 96, 96, 10, 0.8, ['Thank you for saving me!', 'They wanted to bring me to the other prisoners!', 'Here use this is the least I can give you!'], 'samurai', [{ item: 'coin', amount: 1 }, { item: 'string', amount: 1 }]] },
            { x: 2400, type: worldElements.plant_1, elementType: 'blocks', extraArgs: [] },
            { x: 3000, type: worldElements.wall_1, elementType: 'blocks', extraArgs: [] },
            { x: 600, type: worldElements.plant_1, elementType: 'blocks', extraArgs: [] },
            { x: 2350, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: 2425, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: 2555, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: 2600, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: 5000, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: 5100, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: 5150, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: 5450, y: 560, type: worldElements.NPC, elementType: 'entities', extraArgs: ['/img/passiveEntities/beggar.png', 34, 34, 5, 0.4, ['Thanks, man!']] },
            { x: 5530, y: 560, type: worldElements.NPC, elementType: 'entities', extraArgs: ['/img/passiveEntities/villager.png', 34, 34, 5, 0.4, ['Nice!']] },
            { x: 5300, y: 560, type: worldElements.NPC, elementType: 'entities', extraArgs: ['/img/passiveEntities/stranger.png', 32, 32, 4, 0.4, ['Those goblins probably wanted to eat us!!!', 'Her a little gift for saving us!'], 'stranger', [{ item: 'coin', amount: 1 }]] },
            { x: 5200, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
        ]
    },
    mountain: {
        background: {
        ...
        elements: [
            { x: -200, y: StaticPositions.OnGround, type: worldElements.tree_1, elementType: 'blocks', extraArgs: [] },
            { x: 200, y: StaticPositions.OnGround, type: worldElements.tree_1, elementType: 'blocks', extraArgs: [] },
            { x: 400, y: StaticPositions.OnGround, type: worldElements.goblin, elementType: 'entities', extraArgs: [] },
            { x: 800, y: StaticPositions.OnGround, type: worldElements.skeleton, elementType: 'entities', extraArgs: [] },
        ]
    }
};
function changeWorld(world) {
    ...
    blocks.forEach(block => {
        if (block.worldElem) {
            worlds[currentLayer].elements.push(worldElements[block.worldElem]);
        }
    });
    blocks = [];
    entities.forEach(entity => {
        if (entity.worldElem && entity.type.name !== 'player') {
            worlds[currentLayer].elements.push(worldElements[entity.worldElem]);
        }
    });
    entities = [];
    containers.forEach(container => {
        if (container.worldElem) {
            worlds[currentLayer].elements.push(worldElements[container.worldElem]);
        }
    });
    nonWorldElems = [];
    containers = [];
