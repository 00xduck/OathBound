...
class Player implements entity {
    ...
    constructor(x: number, y: number) {
        let skin = (localStorage.getItem('skinPath') as string) || 'img/skins/player.png'
    ...
    update() {
        ...
        if (player.sprite.currentState !== 'attack1' && player.sprite.currentState !== 'attack3') player.data.isAttacking = false
        ...
        const foundBlock = worlds[currentWorld].elements.find(el => {
            ...
            if (!checkCollision(
                ...
                { hitbox: this.sprite.hitbox, pos: this.pos }
            )) return false
    ...
    takeHit(damage: number): void {
        if (!this.data.isAttacking) this.changeState('take_hit')
        if (this.data.immune) return
        let protection = 100
        ...
        if (menu.checkSetting('Master Sound')) playSound('takeDamage', (menu.values.effects / 100) / 2, true)
    ...
    drop() {
        if (this.data.dragging) {
            ...
            if (menu.checkSetting('Master Sound')) playSound('drop', menu.values.effects / 100, true)
        }
    ...
    attack(): void {
        ...
        if (!this.data.onCooldown && !this.data.isAttacking) {
            ...
            if (this.data.onGround) {
                ...
                if (menu.checkSetting('Master Sound')) playSound('slice.mp3', menu.values.effects / 100)
    ...
    addItem(item: item, amount: number) {
        ...
        if (fullInv) grantAchievement('hoarder')
    ...
    removeItems(items: { item: item, amount: number }[]): boolean {
        let success = true
        items.forEach(item => {
            let counter = 0
            for (let y = 0; y < player.data.inventory.length; y++) {
    ...
    updateHealthbar() {
        const filler = document.querySelector('#healthbar-fill') as HTMLElement
    ...
    updateMana() {
        const div = document.querySelector('#manaBar-fill') as HTMLElement;
