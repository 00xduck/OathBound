...
class quest {
    event: event
    entities: entity[]
    constructor(event: event, entities: entity[]) {
        this.event = event
        ...
        if (this.event === 'kill') {
            ...
            this.entities.forEach(innerEntity => {
                entities.forEach(entity => {
                    if (entity === innerEntity) wasKilled = false
                })
            })
...
class NPC extends Entity implements entity {
    ...
    constructor(x: number, y: number, worldElem: worldElementNames, pathToImage: string, spriteWidth: number, spriteHeight: number, frameAmount: number, scale: number, conversation: String[], name: string, present: PresentItem[], id: number) {
        let image = new Image()
