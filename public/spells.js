...
class healSpell {
    ...
    async castSpell() {
        if (player.data.mana < this.manaCost)
            return;
    ...
    async checkEndOfCooldown(frameAmount) {
    ...
    async cooldown() {
        ...
        await new Promise(resolve => setTimeout(resolve, this.cast.castTime));
    }
}
class iceSpikeSpell {
    constructor() {
    ...
    async castSpell() {
        if (player.data.mana < this.manaCost || player.data.onSpellCooldown)
            return;
