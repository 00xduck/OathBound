...
class quest {
    ...
    checkCompletion() {
        if (this.event === 'kill') {
            let wasKilled = true;
            this.entities.forEach(innerEntity => {
...
// enemy classes
class Entity {
    constructor(x, y, maxHealth, attackRange, spriteWidth, spriteHeight, attackDamage, animationStates, img, type, scale, lootDrop, worldElem, name, id) {
        this.x = x;
        this.y = y;
        this.frames = 0;
        this.frameLoc = 0;
...
class skeleton extends Entity {
    constructor(x, y, worldElem, id) {
        ...
        }, scale, [{ amount: 1, drop: "stone", chance: 75 }, { amount: 1, drop: "stone", chance: 75 }], worldElem, 'skeleton', id); // grabs x and y from the parent class
        this.worldElem = worldElem;
        this.init();
    }
    interact() {
...
class trader extends Entity {
    constructor(x, y, trade, worldElem, id) {
        let image = new Image();
...
class NPC extends Entity {
    constructor(x, y, worldElem, pathToImage, spriteWidth, spriteHeight, frameAmount, scale, conversation, name, present, id) {
        let image = new Image();
    ...
    worlds[currentWorld].elements.forEach((element, i) => {
        ...
        if (element.x >= VIEW_LEFT - 200 && element.x <= VIEW_RIGHT + 200) {
            if (element.elementType === 'blocks') {
                ...
                if (element.y) {
                    entities.push(new element.type.class(element.x - levelPos, element.y, ...element.type.args, ...element.extraArgs, entityID));
                    entityID++;
                }
                else {
                    entities.push(new element.type.class(element.x - levelPos, ...element.type.args, entityID));
                    entityID++;
                }
            }
