...
class particle implements particle {
    ...
    update() {
        if (this.entity.data.Xdirec === 1) {
            this.x = this.entity.pos.x + this.entity.sprite.spriteWidth
        } else {
            this.x = this.entity.pos.x + this.entity.sprite.spriteWidth / 2
        }
        if (this.entity.type.name === 'goblin') {
            this.x += 30
        }

        this.y = this.entity.pos.y + this.entity.sprite.spriteHeight
    ...
    draw() {
        ...
        ctx!.drawImage(this.img, frameX, frameY, this.spriteWidth, this.spriteHeight, this.x, this.y, 150 * (this.entity.sprite.scale ?? 1), 150 * (this.entity.sprite.scale ?? 1))
    }
...
class chest implements container {
    ...
    update() {
        ...
        const distanceXToPlayer = Math.abs((player.pos.x + player.sprite.spriteWidth / 2) - (this.pos.x + (this.spriteWidth / 2) * this.scale))
...
abstract class Entity {
    ...
    update(): void {
        ...
        if (this.type.interactable) {
            let distanceXToPlayer = Math.abs((player.pos.x + player.sprite.spriteWidth / 2) - (this.pos.x + (this.sprite.spriteWidth / 2) * this.sprite.scale))
...
class goblin extends Entity implements entity {
    ...
    constructor(x: number, y: number, worldElem: worldElementNames, id: number) {
        ...
        super({ x, y },
            {
                img: image, spriteWidth: 150, spriteHeight: 150, scale: 1,
                animationStates: [
                    {
                        name: 'idle',
                        frames: 4
...
class NPC extends Entity implements entity {
    ...
    constructor(pos: { x: number, y: number }, sprite: { pathToImage: string, spriteWidth: number, spriteHeight: number, frameAmount: number, scale: number }, worldElem: worldElementNames, conversation: String[], name: string, present: PresentItem[], id: number, quest: quest | null, story: { action: actionType, ids: number[], dim: worldName, extra?: any } | null) {
        let image = new Image()
        ...
        super({ x: pos.x, y: pos.y },
            {
                img: image, spriteWidth: sprite.spriteWidth, spriteHeight: sprite.spriteHeight, scale: sprite.scale, animationStates: [{ name: 'idle', frames: sprite.frameAmount }],
            },
    ...
    endConversation() {
        ...
        if (this.story) {
            if (this.story.action === 'destroy') {
                removeWorldElements(this.story.ids, ['id'], this.story.dim)
            } else if (this.story.action === 'spawn') {
                spawnElements(this.story.extra, this.story.dim)
            }
        }
    }
            ...
            if (this.data.onGround) {
                ...
                setTimeout(() => {
                    worlds[currentWorld].elements.forEach(obj => { // get each entity in range of the attack
                        ...
                        const itemKey = this.data.inventory[3][this.data.selectedSlot - 1]
                        const item = itemKey ? items[itemKey] : null
                        let rangeValue = (item?.attackRange ?? 150)
                        if (this.data.Xdirec === 2) rangeValue + 30
                        const distance = (obj.pos.x + (obj.sprite.spriteWidth / 2)) - (this.pos.x + (this.sprite.spriteWidth / 2))

                        if (((this.data.Xdirec === 1 && distance >= 0) || (distance < 0 && this.data.Xdirec === 2)) && Math.abs(distance) <= rangeValue && obj.type.attackable) { // Xdirec === 1 --> rechts Xdirec === 2 --> Links
...
// world logic
type worldElementNames = /* 'bush_fruit' | */ 'invisWall' | 'barrel' | 'crate' | 'player' | 'house_1' | 'house' | 'door_1' | 'teleporter' | 'trader' | 'wall_2' | 'wall_1' | 'goblin' | 'nightBorn' | 'skeleton' | 'tree_1' | 'tree_2' | 'rocks_1' | 'bush_1' | 'bush_2' | 'bush_3' | 'plant_1' | 'statue_1' | 'chest' | 'NPC'
...
const worlds: Record<string, {
    ...
    jungle: {
        ...
        elements: [
            ...
            new NPC({ x: 2700, y: 560 }, { pathToImage: '/img/passiveEntities/stranger.png', spriteWidth: 32, spriteHeight: 32, frameAmount: 4, scale: 0.4 }, 'NPC', ['The skeleton wanted to take me to the other prisoners!', 'If you want to save them you will need some good weapons', 'Here a little gift for saving me!'], 'stranger', [{ item: 'stone', amount: 1 }], 5, null, null),
            ...
            new NPC({ x: 3800, y: 560 }, { pathToImage: '/img/passiveEntities/beggar.png', spriteWidth: 34, spriteHeight: 34, frameAmount: 5, scale: 0.4 }, 'NPC', ['Thanks, man!'], 'beggar', [], 6, null, null),
            new NPC({ x: 3950, y: 430 }, { pathToImage: '/img/passiveEntities/samurai.png', spriteWidth: 96, spriteHeight: 96, frameAmount: 10, scale: 0.8 }, 'NPC', ['Thank you for saving me!', 'Here use this! It is the least I can give you!'], 'samurai', [{ item: 'coin', amount: 1 }], 6, null, null),
            ...
            new NPC({ x: -2000, y: 560 }, { pathToImage: '/img/passiveEntities/villager_1.png', spriteWidth: 34, spriteHeight: 34, frameAmount: 5, scale: 0.4 }, 'NPC', ['Why does everybody leave their garbage on the street?!'], 'mopper', [], -2, null, null),
            ...
            new NPC({ x: -3000, y: 555 }, { pathToImage: '/img/passiveEntities/adventurer.png', spriteWidth: 32, spriteHeight: 34, frameAmount: 4, scale: 0.4 }, 'NPC', ['Please, help us!', 'We need to stop the goblins!', 'Here use this to defend yourself!', 'Now defeat the goblins!'], 'elder', [{ item: 'peasants_robe', amount: 1 }], -2, new quest('kill', [21, 22, 23, 24, 25, 26, 27], 'Defend the village from the goblins', [{ item: 'iron_ingot', amount: 1 }, { item: 'coin', amount: 2 }]), {
                action: 'spawn', ids: [], dim: 'jungle', extra: [
                    new goblin(-300, StaticPositions.OnGround, 'goblin', 21),
                    new goblin(-350, StaticPositions.OnGround, 'goblin', 22),
                    new goblin(-380, StaticPositions.OnGround, 'goblin', 23),
                    new goblin(-400, StaticPositions.OnGround, 'goblin', 24),
                    new goblin(1400, StaticPositions.OnGround, 'goblin', 25),
                    new goblin(1500, StaticPositions.OnGround, 'goblin', 26),
                    new goblin(1600, StaticPositions.OnGround, 'goblin', 27),
                    new goblin(1550, StaticPositions.OnGround, 'goblin', 28)]
            })
        ]
    },
    mountain: {
    ...
    house_1: {
        ...
        elements: [
            ...
            new NPC({ x: 500, y: 560 }, { pathToImage: '/img/passiveEntities/shady_guy.png', spriteWidth: 34, spriteHeight: 34, frameAmount: 5, scale: 0.4 }, 'NPC', ['I overheard some knights!', 'They were talking about an invasion!', 'An invasion from the goblins attacking our very village!', 'They even wanted to desert!'], 'idk', [], -2, null, { action: 'destroy', ids: [30], dim: 'jungle' }),
